...
def single_record(ctrlno):
    ...
    if record_format == 'xml':
        ...
        if record is None:
            record_html, holdings_html, holdings = "<h1>Record ID not found</h1>", '', []
        else:
        ...
        return render_template('record.html',
            ctrlno=ctrlno, record_html=Markup(record_html),
            holdings=holdings)
...
def guide():
    record_count = f"{Record.query.count():,d}"
    rel_count = f"{RecordRel.query.count():,d}"
