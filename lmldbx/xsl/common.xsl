      ...
      <xsl:for-each select="*">
        <xsl:choose>
          <xsl:when test="name(.)='part'">
            <!-- name part -->
            <a><xsl:copy-of select="../@href"/><xsl:copy-of select="../@title"/><xsl:apply-templates select="@lang"/><xsl:value-of select="."/></a>
          </xsl:when>
          <xsl:otherwise>
            <!-- qualifiers -->
...
<xsl:template name="time-content-elements">
  <a><xsl:copy-of select="@href"/><xsl:copy-of select="@title"/>
    <xsl:choose>
...
<xsl:template match="xobis:calendar">
  (<a><xsl:copy-of select="@href"/><xsl:copy-of select="@title"/><xsl:value-of select="@title"/></a>)
</xsl:template>
